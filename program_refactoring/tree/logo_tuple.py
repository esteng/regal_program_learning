...
from program_refactoring.tree.node import Node, LogoNode
...
from program_refactoring.domains.logos.utils import get_func_names, convert_to_ascii 
from program_refactoring.codebank.codebank import CodeBank
from program_refactoring.codebank.test_case import TestCase, LogoTestCase
...
"""
    ...
    def merge(self, codebank, model, done = [], do_retry = True, round_added: int = None, helpers_first: bool = True,
              use_self_consistency: bool = False, self_consistency_width: int = 3):
        ...
        prompt = logo_tuple_refactor_prompt.format(codebank_instr=codebank_instr,
        ...
        for idx, funcs in funcs_called_by_idx.items():
            ...
            for f in funcs: 
                ...
                if f in codebank._codebank.keys():
                    ...
                    # add test case 
                    left_tc = LogoTestCase(new_nodes_by_idx[idx], 
    ...
    def test_programs(self, 
        ...
        for idx, prog in new_programs.items():
            ...
            prog_dict = {k:v for k,v in self.nodes[idx].__dict__.items() if k in LogoNode.__init__.__code__.co_varnames}
            prog_dict['type'] = "pred"
